...
export const useCookie = () => {
  ...
  const saveCookie = useCallback(
    (cookieName: string, value: string, options = defaultOptions) => {
  ...
  const getCookieValue = (cookieName: string): string => {
    ...
    const isPotentialCookieValueAvailable =
      typeof potentialCookieValue === 'string';
  ...
  const deleteCookie = (cookieName: string) => {
    ...
    const deletionCookieDetails = stringifyCookieDetails(
      cookieName,
      cookieValue,
