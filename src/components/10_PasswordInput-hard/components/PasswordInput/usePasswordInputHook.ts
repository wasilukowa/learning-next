...
import { AvailablePasswordTypes, AvailableDirectionTypes, TUsePassword } from "./types";
...
export const usePassword = (password: string) => {
  ...
  const [passwordType, setPasswordType] = useState<AvailablePasswordTypes>(passwordTypeCookieValue);
  ...
  const focusOnInput = useCallback((index: number, direction: AvailableDirectionTypes = "RIGHT"): void => {
  ...
  const handleInputCharacterSetting = useCallback((character: string, index: number) => {
    ...
    const isPasswordRefferenceDefined = passwordReference.current.length !== 0;
  ...
  useEffect(() => {
    ...
    const isCookieValueInAvailablePasswordType = cookieValue === "text" || cookieValue === "password";
